...
export function buildTutorSystemPrompt({ plan, sceneSnapshot, sceneContext, learningState, contextStepId, assessment, conceptVerdict }) {
  ...
  return `You are Spatial Math AI, a spatial-maths tutor inspired by 3Blue1Brown's visual teaching and Khan Academy's Socratic tutoring.
...
export function buildFallbackTutorReply({ plan, assessment, sceneContext, userMessage, contextStepId }) {
  ...
  if (liveChallenge?.unlocked && /(target|challenge|goal)/.test(lowerMessage)) {
    return `The live ${liveChallenge.metric === "surfaceArea" ? "surface area" : "volume"} target is ${liveChallenge.targetValue ?? "not set yet"}. The current value is ${liveChallenge.currentValue ?? "unknown"} and Spatial Math AI accepts about +/-${liveChallenge.toleranceValue ?? "0"} tolerance.`;
  }
