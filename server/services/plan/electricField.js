...
function buildPlanFields(_questionText, _sourceSummary = {}, subtype) {
  if (subtype === "flux") {
    return {
      overview: "Spatial Math AI built an electric-field playground with a Gaussian surface so the learner can see flux as field lines crossing a boundary.",
      sceneFocus: {
  ...
  if (subtype === "dipole") {
    return {
      overview: "Spatial Math AI built an electric dipole so the learner can drag the charges and watch the field bend between them in real time.",
      sceneFocus: {
  ...
  return {
    overview: "Spatial Math AI built a single-charge field scene so the learner can connect arrow direction, density, and particle motion to electric force.",
    sceneFocus: {
